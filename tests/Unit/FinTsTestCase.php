...
use BytesCommerce\Tests\FinTsPeer;
...
abstract class FinTsTestCase extends TestCase
{
    ...
    public const TEST_BANK_CODE = '11223344';
    ...
    protected ?\PHPUnit\Framework\MockObject\MockObject $connection = null;
    ...
    {
        // We mock random_int() for the $randomReference generation in Message.php.
        $mockObject = $this->getFunctionMock('BytesCommerce\Protocol', 'random_int');
        $mockObject->expects($this->any())->with(1000000, 9999999)->willReturn(9999999);
        ...
        $timeMock = $this->getFunctionMock('BytesCommerce\Segment\HNVSK', 'time');
    ...
    protected function setUpConnection()
    {
        ...
        $this->connection->expects($this->any())->method('send')->willReturnCallback(function ($request): string {
            ...
            if ((string) $expectedRequest !== '' && !str_starts_with($expectedRequest, 'HNHBK')) {
