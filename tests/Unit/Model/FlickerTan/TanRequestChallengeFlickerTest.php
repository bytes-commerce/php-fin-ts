<?php

declare(strict_types=1);

namespace BytesCommerce\Tests\Unit\Model\FlickerTan;

use BytesCommerce\Model\FlickerTan\TanRequestChallengeFlicker;
use BytesCommerce\Syntax\Bin;
use PHPUnit\Framework\TestCase;

class TanRequestChallengeFlickerTest extends TestCase
{
    /**
     * the startcodes with expected hex values
     */
    private const SC_OLD_VERSION = '11 04 871 49552 05 123456789F 14 302C3031 07';

    private const SC_BESTAND_ABFRAGEN_IN1 = '038 8A 01 2392302069 22 DE12500105170648489890';
     // example iban
    private const HEX_BESTAND_ABFRAGEN_OUT1 = '1f8501239230206956444531323530303130353137303634383438393839305e';

    private const SC_BESTAND_ABFRAGEN_IN2 = '038 8A 01 2392307899 22 DE12500105170648489890';
     // example iban
    private const HEX_BESTAND_ABFRAGEN_OUT2 = '1f8501239230789956444531323530303130353137303634383438393839300c';

    public function testGetHexOldVersion(): void
    {
        $this->expectException(\InvalidArgumentException::class);
        new TanRequestChallengeFlicker(new Bin(self::SC_OLD_VERSION));
    }

    public function testGetHex1(): void
    {
        $tanRequestChallengeFlicker = new TanRequestChallengeFlicker(new Bin(self::SC_BESTAND_ABFRAGEN_IN1));
        $this->assertEquals(self::HEX_BESTAND_ABFRAGEN_OUT1, $tanRequestChallengeFlicker->getHex());
    }

    public function testGetHex2(): void
    {
        $tanRequestChallengeFlicker = new TanRequestChallengeFlicker(new Bin(self::SC_BESTAND_ABFRAGEN_IN2));
        $this->assertEquals(self::HEX_BESTAND_ABFRAGEN_OUT2, $tanRequestChallengeFlicker->getHex());
    }
}
