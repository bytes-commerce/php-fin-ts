...
namespace BytesCommerce\Tests\Unit\Integration\Atruvia;

use BytesCommerce\Action\SendSEPATransfer;
use BytesCommerce\Model\VopVerificationResult;
use BytesCommerce\Segment\VPP\HIVPPv1;
...
class SendTransferVoPTest extends AtruviaIntegrationTestBase
{
    ...
    public function testVopWithResultMatchButConfirmationRequired(): void
    {
        ...
        $this->assertFalse($sendSEPATransfer->needsVopConfirmation());
        $this->assertFalse($sendSEPATransfer->needsTan());
        $this->assertFalse($sendSEPATransfer->isDone());
        $this->assertEquals(2, $sendSEPATransfer->getPollingInfo()->getNextAttemptInSeconds());
        ...
        $this->assertFalse($sendSEPATransfer->needsPollingWait());
        $this->assertTrue($sendSEPATransfer->needsVopConfirmation());
        $this->assertFalse($sendSEPATransfer->needsTan());
        $this->assertFalse($sendSEPATransfer->isDone());
        $this->assertEquals(
            VopVerificationResult::CompletedFullMatch,
            $sendSEPATransfer->getVopConfirmationRequest()->getVerificationResult()
        );
        ...
        $this->fints->confirmVop($sendSEPATransfer);
        $this->assertFalse($sendSEPATransfer->needsPollingWait());
        $this->assertFalse($sendSEPATransfer->needsVopConfirmation());
        $this->assertTrue($sendSEPATransfer->needsTan());
        $this->assertFalse($sendSEPATransfer->isDone());
        $this->assertEquals(
            'Bitte bestätigen Sie den Vorgang in Ihrer SecureGo plus App',
            $sendSEPATransfer->getTanRequest()->getChallenge()
        );
        ...
        $this->fints->checkDecoupledSubmission($sendSEPATransfer);
        $this->assertFalse($sendSEPATransfer->needsPollingWait());
        $this->assertFalse($sendSEPATransfer->needsVopConfirmation());
        $this->assertFalse($sendSEPATransfer->needsTan());
        $this->assertTrue($sendSEPATransfer->isDone());
