...
use BytesCommerce\Segment\VPA\HKVPAv1;
...
class VopHelper
{
    ...
    public static function createHKVPPForInitialRequest(BPD $bpd): HKVPPv1
    {
        ...
        $hkvpPv1 = HKVPPv1::createEmpty();
        $hkvpPv1->unterstuetztePaymentStatusReports->paymentStatusReportDescriptor = $supportedFormats;
        return $hkvpPv1;
    }

    /**
     * @param BPD $bpd The BPD.
     * @param VopPollingInfo $vopPollingInfo The polling info we got from the immediately preceding request.
     * @return HKVPPv1 A segment to poll the server for the completion of Verification of Payee.
     */
    public static function createHKVPPForPollingRequest(BPD $bpd, VopPollingInfo $vopPollingInfo): HKVPPv1
    {
        $hkvpPv1 = static::createHKVPPForInitialRequest($bpd);
        $hkvpPv1->aufsetzpunkt = $vopPollingInfo->getAufsetzpunkt();
        $hkvpPv1->pollingId = $vopPollingInfo->getPollingId();
        return $hkvpPv1;
    }

    /**
     * @param Message $message The response we just received from the server.
     ...
     */
    public static function checkPollingRequired(Message $message, int $hkvppSegmentNumber): ?VopPollingInfo
    {
        ...
        $aufsetzpunkt = $message->findRueckmeldung(Rueckmeldungscode::AUFSETZPUNKT, $hkvppSegmentNumber);
        if (!$aufsetzpunkt instanceof \BytesCommerce\Segment\HIRMS\Rueckmeldung) {
            return null;
        }

        /** @var HIVPPv1 $hivpp */
        $hivpp = $message->findSegment(HIVPPv1::class);
    ...
    public static function checkVopConfirmationRequired(
        ...
        $codes = $message->findRueckmeldungscodesForReferenceSegment($hkvppSegmentNumber);
    ...
    /**
     * @param VopConfirmationRequestImpl $vopConfirmationRequestImpl The VOP request we're confirming.
     * @return HKVPAv1 A HKVPA segment that tells the bank the request is good to execute.
     */
    public static function createHKVPAForConfirmation(VopConfirmationRequestImpl $vopConfirmationRequestImpl): HKVPAv1
    {
        $hkvpAv1 = HKVPAv1::createEmpty();
        $hkvpAv1->vopId = $vopConfirmationRequestImpl->getVopId();
        return $hkvpAv1;
    }
}
